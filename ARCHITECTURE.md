...
- A writable Codex turn is representable only by an existing physical `directory/**` task declaration. Exact-file and other glob shapes stay read-only instead of being widened; declarations that equal, contain, or enter Git metadata, ignored runtime authority, Codex/DevWeave host composition, or canonical ExecPlan storage are rejected by both the plan contract and the shared approval/sandbox resolver. Approval checks also reject traversal and symlink/junction components.
