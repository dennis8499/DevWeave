...
5. Invoke `grill-me`/`grilling` for material requirements decisions and follow [native-question-contract.md](native-question-contract.md). Before current G2, use Plan Mode as the formal native-question entry; when `request_user_input` is visible, send exactly one question with two or three mutually exclusive options, the recommended option first marked `(Recommended)`, descriptions, and host `Other`. If a pre-G2 ordinary-mode Skill cannot see the tool, stop and request Plan Mode; only an unavailable mode switch or explicit compatibility choice permits the same structured numbered fallback. Wait for the user's answer before recording the next decision.
...
Complete when scope validation passes and G1 is recorded against the current brief, requirements, risk, scope, and knowledge context fingerprint. Stop at the Gate if approval is absent or a material question remains unanswered. G1 approval may be collected through the native Gate question, but the existing explicit `approve` contract remains authoritative. If the user changes a decision after the summary, update the artifact, revalidate, and present the revised Gate summary.
