...
- Extension 只產生使用手冊列出的八個公開 `$devweave` 對話命令；使用者先檢查公開命令 preview，再確認複製到 Codex Chat。
...
4. 在 Dashboard 檢視 gates、phase、tasks、evidence、Wiki health 與 next safe action；這些資料與檔案開啟入口維持唯讀。
5. 在「產生 Codex 對話命令」表單選擇 `new`、`feature`、`refactor`、`bug`、`next`、`status`、`revise` 或 `approve`，填入欄位後先預覽，再按「Confirm and copy」複製公開 `$devweave ...` 命令。
