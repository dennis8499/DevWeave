...
function renderReadinessSummary(readiness: ReviewReadiness): string {
  ...
  return `<div class="readiness-banner ${statusClass}"><div><strong>${escapeHtml(readiness.summary)}</strong><p>目前 gate：${escapeHtml(presentGate(readiness.gate))} · projection-only；engine observation：${escapeHtml(snapshot?.engineGateStatus ?? "unavailable")}，approve 前仍由 engine 驗證。</p></div><span class="pill ${statusClass}">${escapeHtml(statusLabel)}</span></div>`;
}
...
function renderKnowledgeSection(): string {
  ...
  return `<section class="section-card section-intro"><p class="eyebrow">知識</p><h2>讓 Wiki 告訴你哪些內容需要更新</h2><p class="muted">列表來自目前 bounded snapshot；搜尋與分類只搜尋已讀取投影，不觸發額外 repository scan。</p></section>${renderKnowledge(knowledge)}`;
}
...
function renderKnowledgeResults(knowledge?: WorkspaceSnapshot["knowledge"]): string {
  ...
  return `<div id="wiki-metrics" class="metric-grid"><div class="metric"><span>頁面</span><strong>${pages.length}</strong></div><div class="metric"><span>篩選結果</span><strong>${filtered.length}</strong></div><div class="metric"><span>預留頁面</span><strong>${current.placeholderPages.length}</strong></div><div class="metric"><span>待 refresh</span><strong>${current.pendingRefresh.length}</strong></div></div><div class="page-list">${visible.map((page) => { const status = page.status ?? "unknown"; const parseErrors = page.parseErrors ?? []; const hash = page.contentHash ?? "unavailable"; return `<button class="page-row" data-action="open" data-path="${escapeAttr(page.path)}"><span class="status-icon">${icon(status)}</span><span><strong>${escapeHtml(page.title)}</strong><small>${escapeHtml(page.path)} · ${escapeHtml(presentStatus(status))} · hash ${escapeHtml(hash.slice(0, 12))}${page.truncated ? " · 已截斷" : ""}${page.verifiedBy ? ` · 已由 ${escapeHtml(page.verifiedBy)} 驗證` : ""}${parseErrors.length ? " · 解析提醒" : ""}</small></span></button>`; }).join("") || `<p class="muted">找不到符合條件的 Wiki 頁面。</p>`}</div>${!wikiSearch.state.showAll && filtered.length > visible.length ? `<button class="secondary" data-action="show-all-wiki">顯示全部 ${filtered.length} 頁</button>` : ""}`;
}
...
function renderEvidence(work: WorkItemProjection): string {
  const evidence = work.evidence.map((item) => {
    ...
    const metrics = item.metrics ? `<details class="raw-details"><summary>Metrics（bounded）</summary><small>duration：${escapeHtml(String(item.metrics.durationMs ?? "unavailable"))} ms · selected：${escapeHtml(String(item.metrics.verification?.selected ?? "unavailable"))} · skipped：${escapeHtml(String(item.metrics.verification?.skipped ?? "unavailable"))} · usage：${escapeHtml(item.metrics.usage?.status ?? "unavailable")}</small></details>` : "";
    return `<article class="evidence-card"><div class="section-heading"><strong>${escapeHtml(item.id)} · ${escapeHtml(item.kind)}</strong><span class="pill ${state === "passed" ? "success" : state === "stale" ? "warning" : "danger"}">${escapeHtml(presentStatus(state))}</span></div><p>${escapeHtml(item.summary || "沒有摘要")}</p><small>涵蓋：${escapeHtml(item.covers.join(", ") || "—")} · tasks：${escapeHtml(item.tasks.join(", ") || "—")}</small>${metrics}${stale ? `<div class="notice warning"><span class="status-icon">!</span><div><strong>這筆 evidence 不能直接視為 current</strong><p>請重新執行對應 verification，再回來 Refresh。</p></div></div>` : ""}${review}${item.rawLog ? `<details class="raw-details"><summary>查看 raw log 路徑</summary><code class="raw-log-path">${escapeHtml(item.rawLog)}</code></details>` : ""}</article>`;
  }).join("");
