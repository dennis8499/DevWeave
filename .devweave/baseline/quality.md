...
- Windows public release certification：本次提供 0.2.2 VSIX 並保留 0.2.1 artifact；認證範圍限定為 Windows x64 build 10.0.26200／25H2、VS Code 1.131.0、Python 3.14.6、Git 2.51.0.windows.1 與目前 Codex host。VS Code 1.90+、Python 3.11+ 僅為技術門檻，不提供 Marketplace、跨平台或舊版 binary rollback 承諾。
...
- `python -B -m unittest discover -s tests -v`：103 項具有效結果，包含 Wiki reserved preflight、bootstrap G1→G3、review/no-update、context currentness、coverage、九種 scaffold、seal、CLI/guard、legacy 與 repository contract coverage；本次普通權限 token 略過一項 symlink containment，必須在同一 Windows build、具 symlink 權限的隔離執行中補驗通過。
- Repository contract tests：13 項通過，包含 single-router Codebase Wiki 閉環、current-only release contract、maintenance-only exclusion、metadata 與 invocation policy 契約。
...
- `vscode-extension/npm.cmd run typecheck`：通過；`npm.cmd run package` 產生並驗證 0.2.2 production bundle，manifest 具 58 個 bootstrap files、119 個 VSIX entries，source hash/length、package／bundle version、required entries、compatibility declarations 與 current artifact SHA-256 全數匹配，0.2.1 artifact 保留。
...
- 完整 103 項 current Windows suite 的 `unit-tests` verification timeout 為 600 秒；EVID-018 final run 以 581.685 秒通過並有一項因 Windows symlink privilege 不可用而 skipped，該案例必須在同一 build、具 symlink 權限的隔離執行中補驗並保存 current evidence。
