...
- Windows public release certification：本次只提供 0.2.1 VSIX；認證範圍限定為 Windows x64 build 10.0.26200／25H2、VS Code 1.131.0、Python 3.14.6、Git 2.51.0.windows.1 與目前 Codex host。VS Code 1.90+、Python 3.11+ 僅為技術門檻，不提供 Marketplace、跨平台或舊版 binary rollback 承諾。
...
- `python -B -m unittest discover -s tests -v`：98 項具有效結果，包含 Wiki reserved preflight、bootstrap G1→G3、review/no-update、context currentness、coverage、九種 scaffold、seal、CLI/guard、legacy 與 repository contract coverage；普通權限 token 略過的 symlink containment 案例必須在同一 Windows build、具 symlink 權限的隔離執行中補驗通過。
...
- `vscode-extension/npm.cmd run typecheck`：通過；`npm.cmd run package` 只產生並驗證 0.2.1 production bundle，manifest 具 58 個 bootstrap files、118 個 VSIX entries，source hash/length、package／bundle version、required entries、compatibility declarations 與 current artifact SHA-256 全數匹配。
...
- 完整 98 項 Windows suite 的 `unit-tests` verification timeout 為 360 秒；普通 token 若略過一項 symlink privilege case，必須在同 build 的具權限隔離環境補跑該案並保存 current evidence。
